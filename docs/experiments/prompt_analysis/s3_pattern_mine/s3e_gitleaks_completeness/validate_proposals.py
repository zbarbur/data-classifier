"""Validate S3-E proposed patterns against the S2 11K WildChat corpus.
...
log = logging.getLogger("s3e_validate")

SPIKE_DIR = Path(__file__).parent.parent.parent / "s2_spike"
CORPUS = SPIKE_DIR / "corpus.jsonl"
PROPOSALS = Path(__file__).parent / "s3e_proposed_patterns.json"
OUT = Path(__file__).parent / "s3e_corpus_validation.json"
...
def main() -> None:
    ...
    for p in proposals:
        try:
            compiled.append((p["name"], re.compile(p["regex"], re.IGNORECASE), p))
