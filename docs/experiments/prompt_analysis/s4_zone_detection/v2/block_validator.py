...
_FUNC_DEF_RE = re.compile(r"^\s*(?:def|function|func|fn|sub)\s+[a-zA-Z_]\w*\s*\(", re.MULTILINE)

# 2. Variable assignment: identifier = value (not [N] = or label: )
#    x = 1  /  result = process(data)  /  self.x = y
_ASSIGNMENT_RE = re.compile(r"^\s*[a-zA-Z_]\w*(?:\.\w+)*\s*(?::?\s*\w+\s*)?=(?!=)\s*\S", re.MULTILINE)

# 3. Import / include / require / using / package
#    import json  /  from x import y  /  #include <stdio.h>
_IMPORT_RE = re.compile(r"^\s*(?:import|from|#include|require|using|package)\s+[\w\"'<{./]", re.MULTILINE)

# 4. Class / struct / enum / interface / trait definitions
#    class Foo:  /  struct Bar {  /  enum Baz {
_CLASS_DEF_RE = re.compile(r"^\s*(?:class|struct|enum|interface|trait)\s+[A-Za-z_]\w*\s*[\(\{:<]", re.MULTILINE)

# 5. Control flow with structural followers
#    if (  /  for (  /  while (  /  else {  /  elif x:  /  } else {
_CONTROL_FLOW_RE = re.compile(r"^\s*(?:if|else\s*if|elif|for|while|switch|match)\s*[\(\{]", re.MULTILINE)

# 6. Method chain: identifier.identifier(
#    obj.method(  /  self.process(  /  response.json(
_METHOD_CHAIN_RE = re.compile(r"\b[a-zA-Z_]\w*\.[a-zA-Z_]\w*\s*\(")

# 7. Decorator / annotation: @symbol at line start
#    @app.route  /  @Override  /  @dataclass
_DECORATOR_RE = re.compile(r"^\s*@[a-zA-Z_]\w*", re.MULTILINE)

# 8. Preprocessor directives
#    #define  /  #ifdef  /  #pragma
_PREPROCESSOR_RE = re.compile(r"^\s*#(?:define|ifdef|ifndef|endif|pragma|import)\s", re.MULTILINE)

# 9. Return / raise / throw / yield at line start with value or semicolon
#    return x  /  raise ValueError  /  throw new Error  /  yield data
_RETURN_RE = re.compile(r"^\s*(?:return|raise|throw|yield)\s+\S", re.MULTILINE)

# 10. Statement-ending semicolons (strong C-family signal)
#     x = 1;  /  console.log(x);
_SEMICOLON_STMT_RE = re.compile(r"\S.*;\s*$", re.MULTILINE)
...
_FUNC_CALL_RE = re.compile(r"(?<!\\)\b[a-zA-Z_]\w*\(")
...
_R_ASSIGNMENT_RE = re.compile(r"^\s*[a-zA-Z_][\w.]*\s*<-\s*\S", re.MULTILINE)
...
_LATEX_CMD_RE = re.compile(r"\\(?:left|right|frac|cdot|sum|int|sqrt|begin|end|text|mathbb|infty|partial|nabla)\b")

# Unicode math symbols (Greek letters used as variables, operators, etc.)
_UNICODE_MATH_RE = re.compile(r"[∑∏∫∂∇λΔΣΩπμθαβγδεζηξρστφχψω≈≠≤≥±∓∈∉⊂⊃∪∩∞⟹]")
