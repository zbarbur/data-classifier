...
> **Last updated:** 2026-04-15 (Sprint 11 complete — measurement honesty + Sprint 10 cleanup + meta-classifier v3 + family taxonomy as public API)

## Status

| Metric | Value |
|---|---|
| Current sprint | 11 (closed) → 12 (planning) |
...
| Tests | **1518 passing** + 1 skipped (+144 vs Sprint 10) (~67s local with `[meta]`) |
...
| Engines | **5** (column_name, regex, heuristic_stats, secret_scanner, gliner2) + meta-classifier **v3** (shadow, schema v3 with Chao-1 + dictionary-word-ratio features, +0.39 subtype macro F1 over v2). GLiNER2 uses S1 NL-prompt wrapping (Sprint 10) and skips non-text data_type columns (Sprint 10). |
...
| Backlog | Sprint 11 closed with 8 main-session items shipped (all `code_reviewed: true` + `design_reviewed: true`) + 10-phase scanner-tuning-batch shipped via PR #12 on parallel branch. 4 Sprint 12 follow-up items filed from the family A/B analysis: `sprint12-validator-rejected-credential-feature` (P1), `sprint12-has-dictionary-name-match-feature` (P1), `sprint12-retire-date-of-birth-eu-subtype` (P2), `sprint12-shadow-directive-promotion-gate` (P2 stretch). |
...
| **Accuracy (family-level, Sprint 11 new quality gate)** | **cross_family_rate 5.85%** (down from 47.71% prior shadow baseline, 8.2× reduction), **family_macro_f1 0.9286**. Measured on 10,170-shard shadow evaluation via PR #12's canonical family accuracy benchmark. This becomes the primary Sprint 11+ ship gate; subtype F1 is secondary. |
