...
| Gretel synthetic_pii_finance_multilingual | gretelai (HuggingFace) | Apache 2.0 | 56K rows (50k train / 5.6k test), 7 languages (English, Spanish, Italian, French, German, Dutch, Swedish), finance domain only | Sprint 10 ingest (2026-04-14). **The only open corpus where credential labels (`password`, `api_key`, `account_pin`) appear inside long-form financial-document prose** — loan agreements, MT940 statements, SWIFT messages, insurance claims, tax notices. Targeted intervention for the `heuristic_avg_length` corpus-fingerprint shortcut diagnosed in Sprint 9 M1 work: the other credential corpora (SecretBench, gitleaks, detect_secrets) all ship credentials as short isolated tokens, training the meta-classifier to predict CREDENTIAL whenever text is short; this corpus embeds credentials in prose, breaking that correlation. **Schema surprise vs. Gretel-EN**: `pii_spans` is JSON (`json.loads`), not Python repr, and spans are `{start, end, label}` — raw values must be sliced out of `generated_text` by offset. Type map covers 15 of 27 raw labels in the 100-row discovery sample at 100% of mappable-within-existing-vocab labels; 3 labels (`account_pin`, `bban`, `driver_license_number`) are filed for Sprint 11 taxonomy expansion. Source: https://huggingface.co/datasets/gretelai/synthetic_pii_finance_multilingual |
