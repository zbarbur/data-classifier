...
_EXTRA_DIM = 2  # heuristic_dictionary_word_ratio, validator_rejected_credential_ratio
...
def test_feature_schema_version_is_v4():
...
def test_validator_rejected_credential_ratio_is_appended_at_schema_tail():
    # Sprint 12 Item #1: the validator-rejection ratio is the NEW tail
    # slot at index 47 (after dict-word-ratio at 46).
    ...
    assert features[-1] == rejection_ratio
    # Base + one-hot are unchanged (base is zero, one-hot is UNKNOWN).
    assert all(v == 0.0 for v in features[:_BASE_DIM])
    assert features[_one_hot_index("UNKNOWN")] == 1.0
    assert sum(features[_BASE_DIM : _BASE_DIM + _ONE_HOT_DIM]) == 1.0
    # Dict-word-ratio slot (46) is still its default zero.
    assert features[_BASE_DIM + _ONE_HOT_DIM] == 0.0
...
def test_both_extras_coexist_independently():
    # Both extras must be addressable independently — setting one must not
    # leak into the other slot. Catches the symmetric-bug risk where a
    # future extra is appended at the wrong index.
    features = extract_features(
        [],
    ...
    assert features[-1] == 0.8  # index 47
